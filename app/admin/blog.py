...
def blog_categories(page):
    categories = BlogCategory.query.order_by(BlogCategory.created_at.asc()).paginate(
        page, per_page=50
    )
    categories_count = BlogCategory.query.count()
    return render_template(
        "admin/blog/categories/index.html",
...
def blog_category_edit(category_id):
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            return redirect(url_for("admin.blog_categories"))
    return render_template(
        "admin/blog/categories/add-edit.html", form=form, category=category
    )
...
def blog_tags(page):
    ...
    return render_template(
        "admin/blog/tags/index.html", tags=tags, tags_count=tags_count
    )
...
def blog_posts(page):
    ...
    return render_template(
        "admin/blog/posts/index.html", posts=posts, posts_count=posts_count
    )
...
@admin_required
def blog_subs(page):
    subs = BlogNewsLetter.query.order_by(BlogNewsLetter.created_at.asc()).paginate(
        page, per_page=50
    )
    subs_count = BlogNewsLetter.query.count()
    return render_template(
        "admin/blog/subs/index.html", subs=subs, subs_count=subs_count
    )
