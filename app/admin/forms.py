...
class MCategoryForm(FlaskForm):
    ...
    image = FileField(
        "Image", validators=[InputRequired(), FileAllowed(images, "Images only!")]
    )
    ...
    parent = QuerySelectField(
        ...
        query_factory=lambda: db.session.query(MCategory)
...
class TrackingScriptForm(FlaskForm):
    name = StringField(
        "Script Name e.g Hotjar or Google Analytics",
...
class QuestionForm(FlaskForm):
    title = StringField(
        "Question Title", validators=[DataRequired(), Length(min=2, max=25)]
    )
