...
def index(page):
    ...
    categories = (
        BlogCategory.query.order_by(BlogCategory.is_featured.desc())
        .order_by(BlogCategory.order.desc())
        .limit(10)
        .all()
    )
    tags = BlogTag.query.order_by(BlogTag.created_at.desc()).limit(10).all()
    return render_template(
        "blog/index.html", posts=posts, categories=categories, tags=tags
    )
...
def blog_category(category_id, page):
    ...
    posts = (
        BlogPost.query.filter(BlogPost.categories.any(id=category.id))
        .order_by(BlogPost.created_at.desc())
        .paginate(page, per_page=40)
    )
    return render_template(
        "blog/categories/category.html", posts=posts, category=category
    )
...
def blog_tag(tag_id, page):
    ...
    posts = (
        BlogPost.query.filter(BlogPost.tags.any(id=tag.id))
        .order_by(BlogPost.created_at.desc())
        .paginate(page, per_page=40)
    )
...
def blog_article(article_id):
    ...
    tags = post.tags
    return render_template(
        "blog/article.html", post=post, tags=tags, categories=categories
    )
...
def add_comment(article_id):
    ...
    if parent_id and parent_id != "0" and parent_id != 0:
        comment = BlogComment(
            post_id=post.id, text=text, user_id=current_user.id, parent_id=parent_id
        )
    else:
        comment = BlogComment(
            post_id=post.id,
